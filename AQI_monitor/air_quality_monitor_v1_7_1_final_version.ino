/*
  ESP32-S3 Air Quality Monitor — V1.7.1
  ...
  Notes:
    ...
    - V1.7.1 adds SCD4X CO2 / temperature / humidity and uses a five-screen carousel.
...
<div class="section">
  ...
  <div class="stats">
    ...
    <div class="stat">
      <div class="title">SGP41 state</div>
      <div class="statv" id="gasstate">--</div>
    </div>
  </div>
...
<div class="section">
  ...
  <div class="row"><span class="label">Monitor</span><span>V1.7.1</span></div>
</div>
...
async function fetchLive(){
  try{
    ...
    $('gasstate').textContent=d.sgp41_has_reading
      ? 'Ready'
      : (
          d.sgp41_online
            ? (
                d.sgp41_conditioning_sec>0
                  ? 'Conditioning '+d.sgp41_conditioning_sec+'s'
                  : 'Warming up'
              )
            : 'Offline'
        );
...
// ============================================================
// WEB DASHBOARD
// Preserved from V1.4.2, version label updated to V1.7.1.
// ============================================================
...
void handleApiLive() {
  ...
  int n = snprintf(
    ...
    // Backward-compatible generic environmental fields now point
    // to the authoritative SHT45 source.
    shtTemperature,
    shtHumidity,

    // Explicit source-named fields for new clients.
    shtTemperature,
    shtHumidity,
...
// Dashboard does not poll this endpoint in V1.7.1.
// ============================================================
...
void printSystemStatus() {
  ...
  if (sgp41Online) {
    Serial.printf(
      "Gas: VOC Index %ld | NOx Index %ld | SRAW %u/%u\n",
      static_cast<long>(vocIndex),
      static_cast<long>(noxIndex),
      srawVoc,
      srawNox
