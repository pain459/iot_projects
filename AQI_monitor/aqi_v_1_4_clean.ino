/*
  ESP32-S3 Air Quality Monitor — V1.4

  Responsibilities:
    - SPS30 particulate measurements
    - 4-screen SSD1306 OLED carousel
    - Wi-Fi + NTP + mDNS (airmonitor.local)
    - Local web dashboard + 60-minute in-memory history
    - Telegram queries (polled every 60 s) + threshold alerts

  Notes:
    - History is intentionally RAM-only and resets after reboot.
    - Telegram polling is deliberately infrequent to keep the web UI responsive.
    - Web dashboard markup/JavaScript is kept unchanged from the working revision.
*/
...
const unsigned long TELEGRAM_POLL_INTERVAL = 60000UL;  // 60 seconds
